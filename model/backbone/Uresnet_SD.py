...
class DistributionUncertainty(nn.Module):
    ...
    def sqrtvar(self, x):
        t = (x.var(dim=0, keepdim=True) + self.eps).sqrt()   #C
        t = t.repeat(x.shape[0], 1) #B*C
        return t

    def forward(self, x, st_Ucertain):

        if (not self.training) or (np.random.random()) > self.p or st_Ucertain==False :
            return x

        mean = x.mean(dim=[2, 3], keepdim=False) #B*C
        std = (x.var(dim=[2, 3], keepdim=False) + self.eps).sqrt() #B*C
...
class UResNet(nn.Module):
    ...
    def featuremaps(self, x, st_Ucertain):
        x = self.conv1(x)
        x = self.pertubration0(x,st_Ucertain)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        x = self.pertubration1(x,st_Ucertain)
        x = self.layer1(x)
        x = self.pertubration2(x,st_Ucertain)
        x = self.layer2(x)
        x = self.pertubration3(x,st_Ucertain)
        x = self.layer3(x)
        x = self.pertubration4(x,st_Ucertain)
        x = self.layer4(x)
        x = self.pertubration5(x,st_Ucertain)

        return x

    def forward(self, x, st_Ucertain=True):
        x = self.featuremaps(x, st_Ucertain)
        x = self.avgpool1(x)
...
def uresnet18(pretrained=False, uncertainty=0.5, **kwargs):
...
def uresnet50(pretrained=False,uncertainty=0.5,**kwargs):
...
def uresnet101(pretrained=False,uncertainty=0.5,**kwargs):
